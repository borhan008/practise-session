...
const surahNames = (data) => {
  ...
  data.map((surah) => {
    let surahClass = "bg-gray-400";
    if (parseInt(surah.id) % 2 == 1) {
      surahClass = "bg-gray-100";
    }
    ...
    surahDiv.innerHTML = `
                <div class="flex items-center text-black py-3 w-full cursor-pointer" onclick="gettingData('${surah.id}')">
                    <div
                        class="w-10 h-10 bg-gray-700 text-center text-white flex items-center justify-center rounded-full mr-2">
                        ${surah.id}
                    </div>
                    <div class="flex-1">
...
const showSurah = (data) => {
  ...
  div.innerHTML = `<div id="surah-title" class="w-full bg-gray-200 p-5 flex justify-between items-center">
              <h3 class="bg-gray-500 px-4 py-2 text-center rounded-full">${data.id}</h3>
              ...
              <h3>${surahType} <span class="bg-gray-500 px-4 py-2 text-center rounded-full">${data.total_verses}</span></h3>
          </div>           
           <div class="sura-verse text-center p-5 bg-gray-300 border-b-2 border-gray-400">
  ...
  data.verses.map((ayat) => {
    ...
    div2.innerHTML = `<div class="sura-verse text-right p-5 bg-gray-100 border-b-2 border-gray-400 flex justify-between items-center ">
               <button onClick=ayatPlay(${data.id},${ayat.id},'play',this) class="bg-gray-700 p-2 rounded-full all-btn text-white disabled:bg-gray-600">Play</button>
               <div>
                  <h3 class="text-4xl arabic">
                      
                      <span>${ayat.text}</span><span class="bg-gray-500 py-1 px-3 text-center rounded-full text-sm mr-4">${ayat.id}</span>
                  </h3>
...
function ayatPlay(surahNum, ayatNum, Isplay, event) {
  ...
  if (Isplay == "play") {
    ...
    event.setAttribute(
      "onclick",
      `ayatPlay(${surahNum},${ayatNum},'Stop',this)`
    );
    ...
    audio.onended = function () {
      ...
      event.setAttribute(
        "onclick",
        `ayatPlay(${surahNum},${ayatNum},'play',this)`
      );
